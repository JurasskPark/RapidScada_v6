...
namespace Scada.Comm.Drivers.DrvTelnetJP
{
    internal class NetworkInformation
    {
        ...
        static string open = Locale.IsRussian ? "Открыт!" : "Open!";
        static string close = Locale.IsRussian ? "Закрыт!" : "Close!";
        ...
        private void Telnet(ref Tag tag)
        {
            ...
            try
            {
                ...
                else // if the IP did not pass for validity, then we will try to get an ip address from DNS
                {
                    ...
                    if (DriverUtils.IsIpAddress(tag.TagIPAddress) == false)
                    {
                        ...
                        result += $@" " + tag.TagIPAddress.ToString() + ":" + tag.TagPort.ToString() +"";
                        result += $@" {bad}";
                ...
                try
                {               
                    IPEndPoint remoteEP = new IPEndPoint(IPAddress.Parse(tag.TagIPAddress), tag.TagPort);
                    Socket state = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                    if (!state.BeginConnect((EndPoint)remoteEP, new AsyncCallback(ConnectCallback), (object)state).AsyncWaitHandle.WaitOne(tag.TagTimeout, false))
                    {
                        state.Close();
