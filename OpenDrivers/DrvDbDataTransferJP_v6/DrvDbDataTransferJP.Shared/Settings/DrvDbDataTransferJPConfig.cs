...
namespace Scada.Comm.Drivers.DrvDbDataTransferJP
{
    ...
    /// </summary>
    public class DrvDbDataTransferJPProject : DeviceConfigBase
    {
        /// <summary>
        /// Initializes a new instance of the class.
        /// </summary>
        public DrvDbDataTransferJPProject()
        {
            SetToDefault();
        }
        ...
        /// <summary>
        /// Gets the target DB connection settings.
        /// </summary>
        public DbConnSettings TargetDbConnSettings { get; private set; }
        ...
        {
            SourceDbConnSettings = new DbConnSettings();
            TargetDbConnSettings = new DbConnSettings();
        ...
        public new bool Load(string fileName, out string errMsg)
        {
            ...
            try
            {
                ...
                try 
                { 
                    XmlNode sourceNode = rootElem.SelectSingleNode("SourceDbConnSettings") ??
                        rootElem.SelectSingleNode("DbConnSettings");
                    SourceDbConnSettings.LoadFromXml(sourceNode);
                } 
                catch 
                { 
                    SourceDbConnSettings = new DbConnSettings(); 
                ...
                try
                {
                    XmlNode targetNode = rootElem.SelectSingleNode("TargetDbConnSettings") ??
                        rootElem.SelectSingleNode("DbConnSettings");
                    TargetDbConnSettings.LoadFromXml(targetNode);
                }
                catch
                {
                    TargetDbConnSettings = new DbConnSettings();
        ...
        public new bool Save(string fileName, out string errMsg)
        {
            try
            {
                ...
                XmlElement rootElem = xmlDoc.CreateElement("DrvDbDataTransferJPProject");
                xmlDoc.AppendChild(rootElem);

                try 
                { 
                    SourceDbConnSettings.SaveToXml(rootElem.AppendElem("SourceDbConnSettings"));
                    TargetDbConnSettings.SaveToXml(rootElem.AppendElem("TargetDbConnSettings"));
                } 
                catch { }
