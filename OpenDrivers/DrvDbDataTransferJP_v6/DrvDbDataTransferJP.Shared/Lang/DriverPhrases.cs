...
namespace Scada.Comm.Drivers.DrvDbDataTransferJP
{
    ...
    {
        public static string ProjectDirRequired { get; private set; }

        // Client
        public static string ProductName { get; private set; } = "DrvDbDataTransferJP";
        ...
        {
            LocaleDict dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransferJP.View.Application");
            ...
            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransferJP.View.Combobox");
            All = dict[nameof(All)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransferJP.View.DialogBox");
            DeleteRow = dict[nameof(DeleteRow)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransferJP.View.Files");
            NamedFileNotFound = dict[nameof(NamedFileNotFound)];
            NotHexadecimal = dict[nameof(NotHexadecimal)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransferJP.View.Format");
            FileDoesNotExist = dict[nameof(FileDoesNotExist)];

            LocaleDict dictionary = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransferJP.View.Forms.FrmProject");
            ProjectDirRequired = dictionary["DrvDbDataTransferJPDictionaries"];
        }
    }
}
