...
SET "PACKAGE_VERSION=6.5.0.1"
SET "PACKAGE_PREFIX=DrvDbDataTransferJP_%PACKAGE_VERSION%"
SET "INSTALL_SCADA_DIR=C:\Program Files\SCADA"

IF /I "%~1"=="--package-only" SET "PACKAGE_ONLY=1"

ECHO ==================
ECHO Building DrvDbDataTransferJP release packages
ECHO ==================
...
IF /I "%LOGIC_RUNTIME%"=="anycpu" (
    DOTNET publish ".\DrvDbDataTransferJP.Logic\DrvDbDataTransferJP.Logic.csproj" -c Release --framework net8.0 --self-contained false --output "%SCADA_DIR%\ScadaComm\Drv"
) ELSE (
    DOTNET publish ".\DrvDbDataTransferJP.Logic\DrvDbDataTransferJP.Logic.csproj" -c Release --framework net8.0 --runtime %LOGIC_RUNTIME% --self-contained false --output "%SCADA_DIR%\ScadaComm\Drv"
)
IF ERRORLEVEL 1 EXIT /B 1
CALL :CleanPluginFolder "%SCADA_DIR%\ScadaComm\Drv"
IF ERRORLEVEL 1 EXIT /B 1
CALL :MoveRootRuntimeFolder "%SCADA_DIR%\ScadaComm\Drv" "DrvDbDataTransferJP.Logic"
IF ERRORLEVEL 1 EXIT /B 1

ECHO COMPILE VIEW for %VIEW_RUNTIME%...
IF /I "%VIEW_RUNTIME%"=="anycpu" (
    DOTNET publish ".\DrvDbDataTransferJP.View\DrvDbDataTransferJP.View.csproj" -c Release --framework net8.0-windows --self-contained false --output "%SCADA_DIR%\ScadaAdmin\Lib"
) ELSE (
    DOTNET publish ".\DrvDbDataTransferJP.View\DrvDbDataTransferJP.View.csproj" -c Release --framework net8.0-windows --runtime %VIEW_RUNTIME% --self-contained false --output "%SCADA_DIR%\ScadaAdmin\Lib"
)
IF ERRORLEVEL 1 EXIT /B 1
...
IF ERRORLEVEL 1 EXIT /B 1
CALL :MoveRootRuntimeFolder "%SCADA_DIR%\ScadaAdmin\Lib" "DrvDbDataTransferJP.View"
IF ERRORLEVEL 1 EXIT /B 1

IF "%BUILD_WINFORM%"=="1" (
    ECHO COMPILE WINFORM for %VIEW_RUNTIME%...
    IF /I "%VIEW_RUNTIME%"=="anycpu" (
        DOTNET publish ".\DrvDbDataTransferJP.Winform\DrvDbDataTransferJP.Winform.csproj" -c Release --framework net8.0-windows --self-contained false --output "%RELEASE_DIR%\App"
    ) ELSE (
        DOTNET publish ".\DrvDbDataTransferJP.Winform\DrvDbDataTransferJP.Winform.csproj" -c Release --framework net8.0-windows --runtime %VIEW_RUNTIME% --self-contained false --output "%RELEASE_DIR%\App"
    )
    IF ERRORLEVEL 1 EXIT /B 1
) ELSE (
...
IF EXIST "%LIB_DIR%\Lang\DrvDbDataTransferJP.*.xml" (
    XCOPY /Y /I "%LIB_DIR%\Lang\DrvDbDataTransferJP.*.xml" "%LANG_DIR%\"
    IF ERRORLEVEL 1 EXIT /B 1
)
...
IF NOT EXIST "%SCADA_DIR%\ScadaAdmin\Lang\DrvDbDataTransferJP.*.xml" (
    ECHO Missing language files in %SCADA_DIR%\ScadaAdmin\Lang
    EXIT /B 1
)
IF NOT EXIST "%SCADA_DIR%\ScadaAdmin\Lib\DrvDbDataTransferJP.View.dll" (
    ECHO Missing View DLL in %SCADA_DIR%\ScadaAdmin\Lib
    EXIT /B 1
)
IF NOT EXIST "%SCADA_DIR%\ScadaAdmin\Lib\DrvDbDataTransferJP.View" (
    ECHO Missing View dependency folder in %SCADA_DIR%\ScadaAdmin\Lib
    EXIT /B 1
)
IF NOT EXIST "%SCADA_DIR%\ScadaComm\Drv\DrvDbDataTransferJP.Logic.dll" (
    ECHO Missing Logic DLL in %SCADA_DIR%\ScadaComm\Drv
    EXIT /B 1
)
IF NOT EXIST "%SCADA_DIR%\ScadaComm\Drv\DrvDbDataTransferJP.Logic" (
