...
namespace Scada.Comm.Drivers.DrvDbImportPlus
{
    public static class DriverPhrases
    {
        ...
        // Client
        public static string ProductName { get; private set; } = "DrvDbImportPlus";
        ...
        {
            LocaleDict dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbImportPlus.View.Application");
            ...
            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbImportPlus.View.Combobox");
            All = dict[nameof(All)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbImportPlus.View.DialogBox");
            DeleteRow = dict[nameof(DeleteRow)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbImportPlus.View.Files");
            NamedFileNotFound = dict[nameof(NamedFileNotFound)];
            NotHexadecimal = dict[nameof(NotHexadecimal)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbImportPlus.View.Format");
            FileDoesNotExist = dict[nameof(FileDoesNotExist)];

            LocaleDict dictionary = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbImportPlus.View.Forms.FrmProject");
            ProjectDirRequired = dictionary["DrvDbImportPlusDictionaries"];
        }
    }
}
