    ...
    public sealed class DdeContext : IDisposable, ISynchronizeInvoke
    {
        ...
        private sealed class DdeThread : IDisposable, ISynchronizeInvoke
        {
            ...
            public object Invoke(Delegate method, object[] args)
            {
                ...
                Task<object> task = Enqueue(method, args);
            ...
            private Task<object> Enqueue(Delegate method, object[] args)
            {
                TaskCompletionSource<object> taskSource = new TaskCompletionSource<object>(TaskCreationOptions.RunContinuationsAsynchronously);
                lock (_WorkItems)
                {
                    _WorkItems.Enqueue(new WorkItem(method, args, taskSource));
                }
