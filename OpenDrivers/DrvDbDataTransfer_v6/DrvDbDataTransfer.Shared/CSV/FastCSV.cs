...
public class FastCSV
{
    ...
    public static string[] AdditionDoubleQuotesArray(string[] strArray)
    {
        string[] strArrayNew = new string[strArray.Length];
        for (int i = 0; i < strArrayNew.Length; i++)
        {
            strArrayNew[i] = ("\"" + strArray[i] + "\"").Trim();
        }
        return strArrayNew;
    }

    public static string[] RemovingDoubleQuotesArray(string[] strArray)
    {
        string[] strArrayNew = new string[strArray.Length];
        for (int i = 0; i < strArrayNew.Length; i++)
        {
            strArrayNew[i] = strArray[i].Replace("\"", "").Trim();
        }
        return strArrayNew;
    }
    ...
    public static List<T> ReadFile<T>(string filename, bool hasheader, char delimiter, string encoding, bool quote, ToOBJ<T> mapper)
    {
        #region Encoding
        Encoding csvEncoding;
        if (encoding == "Unicode") // encoding selected Unicode
        {
            csvEncoding = Encoding.Unicode;
        }
        else if (encoding == "ASCII") // encoding selected ASCII 
        {
            csvEncoding = Encoding.ASCII;
        }
        else if (encoding == "UTF7") // encoding selected UTF7
        {
            #pragma warning disable SYSLIB0001
            csvEncoding = Encoding.UTF7;
            #pragma warning restore SYSLIB0001
        }
        else if (encoding == "UTF8") // encoding selected UTF8
        {
            csvEncoding = Encoding.UTF8;
        }
        else
        {
            csvEncoding = Encoding.Unicode; // set by default Unicode
        }
        #endregion Encoding

        return ReadData(new StreamReader(filename, csvEncoding), hasheader, _COLCOUNT, delimiter, mapper);
    }
    ...
    public static void WriteFile<T>(string filename, string[] headers, char delimiter, string encoding, bool quote, List<T> list, FromObj<T> mapper)
    {
        #region Encoding
        Encoding csvEncoding;
        if (encoding == "Unicode") // encoding selected Unicode
        ...
        {
            using (StreamWriter s = new StreamWriter(f, csvEncoding))
            {
                if (headers != null)
                {
                    if(quote)
                    {
                        headers = AdditionDoubleQuotesArray(headers);
                    }
