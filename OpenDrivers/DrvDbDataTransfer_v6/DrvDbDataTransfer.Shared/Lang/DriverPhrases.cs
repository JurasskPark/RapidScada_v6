...
namespace Scada.Comm.Drivers.DrvDbDataTransfer
{
    public static class DriverPhrases
    {
        ...
        // Client
        public static string ProductName { get; private set; } = "DrvDbDataTransfer";
        ...
        {
            LocaleDict dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransfer.View.Application");
            ...
            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransfer.View.Combobox");
            All = dict[nameof(All)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransfer.View.DialogBox");
            DeleteRow = dict[nameof(DeleteRow)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransfer.View.Files");
            NamedFileNotFound = dict[nameof(NamedFileNotFound)];
            NotHexadecimal = dict[nameof(NotHexadecimal)];

            dict = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransfer.View.Format");
            FileDoesNotExist = dict[nameof(FileDoesNotExist)];

            LocaleDict dictionary = Locale.GetDictionary("Scada.Comm.Drivers.DrvDbDataTransfer.View.Forms.FrmProject");
            ProjectDirRequired = dictionary["DrvDbDataTransferDictionaries"];
        }
    }
}
