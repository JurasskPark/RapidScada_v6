...
namespace Scada.Server.Modules.ModArcMicrosoftSqlJP.Logic
{
    ...
    /// </summary>
    internal class MicrosoftSqlEAL : EventArchiveLogic
    {
        private readonly ModuleConfig moduleConfig; // the module configuration
        private readonly MicrosoftSqlEAO options;        // the archive options
        ...
        private SqlConnection writingConn;       // the database connection for writing
        ...
        /// </summary>
        public MicrosoftSqlEAL(IArchiveContext archiveContext, ArchiveConfig archiveConfig, int[] cnlNums,
            ModuleConfig moduleConfig) : base(archiveContext, archiveConfig, cnlNums)
        {
            ...
            options = new MicrosoftSqlEAO(archiveConfig.CustomOptions);
        ...
        private void CreateDbEntities()
        {
            ...
            try
            {
                ...
                SqlCommand cmd2 = new(queryBuilder.CreateEventTableQuery, conn);
        ...
        public override void DeleteOutdatedData()
        {
            ...
            try
            {
                ...
                conn.Open();
                Stopwatch stopwatch = Stopwatch.StartNew();
                int rowsAffected = DbUtils.DeleteOutdatedData(conn, queryBuilder.EventTable, minDT);
                stopwatch.Stop();
        ...
        public override DateTime GetLastWriteTime()
        {
            ...
            try
            {
                ...
                cmd.Parameters.Add("startTime", SqlDbType.DateTime2).Value = utcNow.AddDays(-1.0);
                cmd.Parameters.Add("endTime", SqlDbType.DateTime2).Value = utcNow;
        ...
        public override Event GetEventByID(long eventID)
        {
            try
            {
                ...
                cmd.Parameters.Add("startTime", SqlDbType.DateTime2).Value = eventTime;
                cmd.Parameters.Add("endTime", SqlDbType.DateTime2).Value = eventTime.AddSeconds(1.0);
                Event ev = null;
        ...
        public override List<Event> GetEvents(TimeRange timeRange, DataFilter filter)
        {
            try
            {
                ...
                string sqlFilter = filter.GetSqlFilter(QueryBuilder.EventColumnMap, "AND (", ")",
                    () => { return new SqlParameter(); }, out List<DbParameter> dbParams);
                sqlFilter = sqlFilter.Replace("false", "0 = 1");
                string endOper = timeRange.EndInclusive ? "<=" : "<";
                string sortOrder = filter.OriginBegin ? "ASC" : "DESC";
                string pagination = filter.Limit > 0
        ...
        public override void AckEvent(EventAck eventAck)
        {
            try
            {
                ...
                SqlCommand cmd = new(sql, writingConn);
                cmd.Parameters.Add("ackTimestamp", SqlDbType.DateTime2).Value = eventAck.Timestamp;
                cmd.Parameters.Add("ackUserID", SqlDbType.Int).Value = eventAck.UserID;
                cmd.Parameters.Add("eventID", SqlDbType.BigInt).Value = eventAck.EventID;
                cmd.Parameters.Add("startTime", SqlDbType.DateTime2).Value = eventTime;
                cmd.Parameters.Add("endTime", SqlDbType.DateTime2).Value = eventTime.AddSeconds(1.0);
